...
void hV_Screen_Buffer::drawSensorFrame(Sensor sensorToDraw, uint8_t frameNumber) {
    uint8_t icons[][] {
        //CO2
        {
            //co2
    ...
    partialScreenBitmap(baseline_x+22, 10, icons[sensorToDraw], 104, 69);
    if(sensorToDraw = temperature) partialScreenBitmap(baseline_x+22, 80, icons[8], 48, 70);
}

void hV_Screen_Buffer::updateFrameVal(uint8_t frameNumber, Sensor sensorToDraw, String sensorReading) {
    uint8_t baseline_x;
    if(frameNumber) baseline_x = 148;
    drawNumBig(sensorReading, baseline_x, 80);
}
